...
async function runSqlQuerySerially(operation) {
    const previous = queryTail;
    let releaseCurrent;
    queryTail = new Promise(resolve => {
...
class SqlDatabase {
     ...
     */
    static async query(text, params = {}, retries = 2) {
        return runSqlQuerySerially(() => SqlDatabase.executeQuery(text, params, retries));
    }
    static async executeQuery(text, params = {}, retries = 2) {
        try {
            ...
            if (isConnectionError) {
                ...
                if (retries > 0) {
                    ...
                    return SqlDatabase.executeQuery(text, params, retries - 1);
                }
            }
    ...
    static async one(text, params = {}) {
        const rows = await this.query(text, params);
        return rows.length ? rows[0] : null;
    }
    // ================= DASHBOARD STATS =================
    static async getDashboardStats() {
        const row = await SqlDatabase.one(`
      SELECT
    ...
    static async addBuilding(payload) {
        const result = await SqlDatabase.query(`
    ...
    `, payload);
        return (await SqlDatabase.getBuildingById(result[0].id));
    }
    static async updateBuilding(id, payload) {
    ...
    static async deleteBuilding(id) {
        ...
        try {
            const result = await new mssql_1.default.Request(transaction).input('id', id).query(`
    ...
    // ================= ROOMS =================
    static async getRooms(buildingId, floor) {
        let query = `
      SELECT
        p.PhongHocID AS id,
    ...
    `;
        ...
        return SqlDatabase.query(query, params);
    }
    static async getRoomById(id) {
        const room = await SqlDatabase.one(`
      SELECT
        p.PhongHocID AS id,
        p.ToaNhaID AS building_id,
        p.SoPhong AS room_number,
    ...
    static async getRoomByQr(qrCode) {
        return SqlDatabase.one(`
      SELECT
        p.PhongHocID AS id,
        p.ToaNhaID AS building_id,
    ...
    static async addRoom(payload) {
        ...
        const res = await SqlDatabase.query(`
    ...
    `, {
        ...
        return (await SqlDatabase.getRoomById(newId));
    }
    ...
    // ================= DEVICES =================
    static async getDevices(categoryId, status) {
        let query = `
      SELECT
        d.ThietBiID AS id,
    ...
    static async getDeviceById(id) {
        const dev = await SqlDatabase.one(`
      SELECT
        d.ThietBiID AS id,
        d.PhongHocID AS room_id,
        d.LoaiThietBiID AS category_id,
    ...
    static async getDeviceByQr(qrCode) {
        return SqlDatabase.one(`
      SELECT
        d.ThietBiID AS id,
        d.PhongHocID AS room_id,
    ...
    `, {
        ...
        const newId = res[0]?.id;
        return (await SqlDatabase.getDeviceById(newId));
    }
    ...
    static async getManualByDeviceId(deviceId) {
        const row = await SqlDatabase.one(`
      SELECT
        h.HuongDanID AS id,
        COALESCE(t.LoaiThietBiID, 1) AS category_id,
    ...
    static async getIncidentReportById(id) {
        const row = await SqlDatabase.one(`
      SELECT
        b.BaoHongID AS id,
        ('INC-' + CONVERT(varchar(8), b.NgayTao, 112) + '-' + RIGHT('000' + CAST(b.BaoHongID AS varchar), 3)) AS report_code,
    ...
    `, {
        ...
        return (await SqlDatabase.getIncidentReportById(newId));
    }
    static async updateIncidentStatus(id, status, technicianId, technicianName, solutionNote) {
        await SqlDatabase.query(`
      UPDATE dbo.BaoHong
      SET
        TrangThai = @status,
    ...
    static async getMaintenanceLogsByDeviceId(deviceId) {
        const rows = await SqlDatabase.query(`
      SELECT
        l.LogBaoTriID AS id,
        l.BaoHongID AS report_id,
    ...
    static formatUser(row) {
        const roleName = row.role_name || 'STUDENT';
        const defaultPerms = types_1.DEFAULT_ROLE_PERMISSIONS[roleName] || [];
        return {
            id: row.id,
    ...
    static async createUser(payload) {
        ...
        const roleRow = await SqlDatabase.one(`
    ...
    static async updateOwnPassword(userId, passwordHash) {
        const rows = await SqlDatabase.query(`
    ...
    static async deleteUser(userId) {
        const pool = await SqlDatabase.connect();
        const transaction = new mssql_1.default.Transaction(pool);
        const systemPasswordHash = await bcryptjs_1.default.hash(`system-deleted-account:${Date.now()}`, 12);
        await transaction.begin();
        try {
            const request = new mssql_1.default.Request(transaction);
