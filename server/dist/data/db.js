...
class Database {
    ...
    static save() {
        try {
            fs_1.default.writeFileSync(DB_FILE, JSON.stringify(this.data, null, 2), 'utf8');
        }
        catch (e) {
    ...
    static getRooms(buildingId, floor) {
        ...
        return rooms.map(r => {
            ...
            const pendingCount = this.data.incident_reports.filter(rep => rep.room_id === r.id && (rep.status === 'PENDING' || rep.status === 'IN_PROGRESS' || rep.status === 'ASSIGNED')).length;
            return {
                ...r,
    ...
    static getRoomById(id) {
        ...
        const pendingReportsCount = this.data.incident_reports.filter(rep => rep.room_id === id && (rep.status === 'PENDING' || rep.status === 'IN_PROGRESS' || rep.status === 'ASSIGNED')).length;
        return { ...room, building_code: b ? b.building_code : '', devices, pendingReportsCount };
    }
    static getRoomByQr(qrCode) {
    ...
    static getDevices(categoryId, status) {
        let devices = this.data.devices;
        if (categoryId)
            devices = devices.filter(d => d.category_id === categoryId);
        if (status)
    ...
    static getDevicesByRoomId(roomId) {
    ...
    static createIncidentReport(payload) {
    ...
    static updateIncidentStatus(id, status, technicianName, solutionNote) {
        const report = this.data.incident_reports.find(r => r.id === id);
        if (!report)
            return null;
        report.status = status;
        if (technicianName)
            report.assigned_technician_name = technicianName;
        if (solutionNote)
            report.solution_note = solutionNote;
        if (status === 'RESOLVED') {
    ...
    static addMaintenanceLog(payload) {
