...
].map(([code, name, floors, latitude, longitude, color]) => ({ code, name, floors, latitude, longitude, color })) as BuildingSeed[];

const rooms: RoomSeed[] = [];
const add = (building: string, code: string, name: string, floor: number) => rooms.push({ building, code, name, floor });
const addRange = (building: string, prefix: string, start: number, end: number, floor: number, name = 'Phòng học') => {
  for (let n = start; n <= end; n++) add(building, `${prefix}-${n}`, `${name} ${prefix}-${n}`, floor);
};

add('A1', 'A1-101', 'Giảng đường A1-101', 1); add('A1', 'A1-102', 'Giảng đường A1-102', 1);
...
addRange('A2','A2',101,114,1); [['A2-201','Thư viện',2],['A2-203','Thư viện',2],['A2-204','Thư viện',2],['A2-302','Thư viện',3]].forEach(([c,n,f]) => add('A2', c as string, n as string, f as number));
addRange('A3','A3',101,104,1); addRange('A4','A4',101,111,1);
add('B1','KTH-B','Khu tự học',1); [['GB','Phòng họp giao ban',2],['PHT','Phòng Hiệu trưởng',2],['PPHT','Phòng các phó hiệu trưởng',2],['B1-301','Viện Công nghệ số',3],['CNS','Viện công nghệ số',3],['KTCN','Viện Kỹ thuật Công nghệ',3],['KTTC','Trường kinh tế tài chính',4],['L&QL','Trường luật và quản lý phát triển',4],['SXCT','Phòng sản xuất chương trình',5]].forEach(([c,n,f]) => add('B1', c as string,n as string,f as number));
addRange('C','C',101,104,1,'Phòng máy'); addRange('C','C',201,204,2,'Phòng máy'); addRange('D','D',101,106,1);
[['PH3','Phòng Họp 3',1],['PH4','Phòng Họp 4',1],['TTNN','Trung Tâm Ngoại Ngữ',1],['TTĐTQT','TT Đào Tạo Quốc Tế',1]].forEach(([c,n,f]) => add('E1',c as string,n as string,f as number));
addRange('E1','E1',201,210,2); addRange('E1','E1',301,310,3); addRange('E1','E1',401,410,4);
addRange('E2','E2',101,104,1); addRange('E3','E3',101,104,1);
[['BCSNG','Ban chăm sóc người học',1],['KCNVH','Khoa công nghiệp văn hóa',1],['VPD-H-T','Đoàn thanh niên và hội sinh viên',1],['VĐTSĐH','Viện Đào Tạo Sau Đại Học',1]].forEach(([c,n,f]) => add('F1',c as string,n as string,f as number));
addRange('F2','F2',101,104,1); [['F3-101','Phòng học F3-101'],['F3-102','Dãy phòng F2-102'],['F3-103','Dãy phòng F3-103']].forEach(([c,n]) => add('F3',c,n,1));
...
addRange('H1','H1',101,103,1); addRange('H1','H1',201,204,2); addRange('H1','H1',301,304,3);
...
add('HT1','HT2','Hội Trường 2',2); add('HT1','HT1','Hội Trường 1',1);
for (let floor=1; floor<=4; floor++) addRange('I1','I1',floor*100+1,floor*100+5,floor);
add('I2','I2-101','Khoa kiến thức chung',1); add('I2','I2-102','Khoa kinh tế',1); addRange('I2','I2',103,104,1); for (let floor=2; floor<=4; floor++) addRange('I2','I2',floor*100+1,floor*100+5,floor);
addRange('I3','I3',101,106,1); addRange('I3','I3',201,206,2); [301,302,303,304,306].forEach(n => add('I3',`I3-${n}`,`Phòng học I3-${n}`,3)); add('I3','I3-305','Phòng Học I3-305',1);
addRange('I4','I4',107,113,1); addRange('I4','I4',207,214,2); addRange('I4','I4',307,314,3); addRange('I4','I4',401,402,4);
add('KTH-I4','KTH-I4','Khu tự học I4',1); addRange('K','K',101,106,1); add('K','K-201','Phòng học',2); add('K','TTHTDN&KN','Trung Tâm Hợp Tác Doanh Nghiệp và Khởi Nghiệp',2);
addRange('K23','K23',101,102,1); addRange('K23','K23',201,203,2);
